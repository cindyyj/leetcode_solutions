...
time, O(m*n*log(n)) - O(n*log(n)) sort n logs; *m (compare between two keys, m is th max length of a single log)
space, O(m*n): m is th max length of a single log, n logs
